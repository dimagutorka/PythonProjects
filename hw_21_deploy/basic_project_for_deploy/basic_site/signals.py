...
from basic_site.models import Movies, Rate, Genres, Comments
...
def user_registration_email(sender, instance, created, **kwargs):
	...
	if created:
		...
		send_mail(subject, message=message, from_email=settings.EMAIL_HOST_USER, recipient_list=recipient_list)
...
def user_registration_email(created, instance, **kwargs):
	...
	if created:
		send_mail(subject, message=message, from_email=settings.EMAIL_HOST, recipient_list=recipient_list)
